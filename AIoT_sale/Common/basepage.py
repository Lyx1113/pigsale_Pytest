#存放的是封装的selenium的方法，比如元素点击、元素获取、等待元素的出现、文本框的输入等
...
class BasePage:
    ...
    #删除input时间插件元素中的readonly属性
    def rm_readonly(self,loc):
        """删除时间插件中readonly属性"""
        # js = "$('.app input:eq(4)').removeAttr('readonly')"
        # js = "js=document.documentElement.scrollTop=0"

        self.driver.execute_script("js = '$(document.getElementByName('startTime'))';js.removeAttr('readonly')")
        time.sleep(1)
